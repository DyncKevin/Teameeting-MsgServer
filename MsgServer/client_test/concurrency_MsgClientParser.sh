...
{
    dir=$(ls -l ./client_logs | awk '/^-/ {print $NF}')
    for i in $dir
    do
        {
            logName="./client_logs/"$i
            echo "load file name:" $logName
            rest=`$PARSER_PATH/$PARSER_NAME $logName`
        }&
    done
    echo "ParseResult waiting..."
    wait
}
